...
do
  ...
  oldifs="$IFS"
  IFS=$'\n'
  newimagenames=($(curl --silent --location --request GET "${GITLAB_URL}/api/v4/projects/${GITLAB_PROJECT_ID}/registry/repositories/${GITLAB_REGISTRY_ID}/tags/?per_page=500&page=${thispage}" --header "PRIVATE-TOKEN: ${GITLAB_AUTH_TOKEN}" | jq '.[].name'))
  oldifs="$IFS"
  ...
  nextpage=$(curl -I --silent --location --request GET "${GITLAB_URL}/api/v4/projects/${GITLAB_PROJECT_ID}/registry/repositories/${GITLAB_REGISTRY_ID}/tags/?per_page=500&page=${thispage}" --header "PRIVATE-TOKEN:  ${GITLAB_AUTH_TOKEN}" | grep -Fi X-Next-Page | sed -r 's/X-Next-Page:\ //i' )
...
do
  ...
  created_date=$(curl --silent --location --request GET "${GITLAB_URL}/api/v4/projects/${GITLAB_PROJECT_ID}/registry/repositories/${GITLAB_REGISTRY_ID}/tags/${imagename}" --header "PRIVATE-TOKEN: ${GITLAB_AUTH_TOKEN}" | jq '.created_at')
  ...
  then
      ...
      then
        ...
        if [ ${DRY_RUN} != "TRUE" ]
        then
          curl --silent --location --request DELETE "${GITLAB_URL}/api/v4/projects/${GITLAB_PROJECT_ID}/registry/repositories/${GITLAB_REGISTRY_ID}/tags/${imagename}" --header "PRIVATE-TOKEN: ${GITLAB_AUTH_TOKEN}"
        fi
